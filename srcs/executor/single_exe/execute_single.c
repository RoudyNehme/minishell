...
/*   Updated: 2025/12/16 20:17:40 by rberdkan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	execute_builtin_single(t_cmd *cmd, t_shell *shell, char *line,
		t_token *tokens)
{
	int	save_stdout;
	int	save_stdin;
	...
	shell->last_exit_status = run_builtin(cmd->args, shell, line, tokens,
			shell->cmds);
...
void	execute_single(t_cmd *cmd, t_shell *shell, char *line, t_token *tokens)
{
	if (is_builtin(cmd->args[0]))
		execute_builtin_single(cmd, shell, line, tokens);
	else
